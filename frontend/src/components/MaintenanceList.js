...
const MaintenanceList = () => {
  ...
  const handleAddMaintenance = async () => {
    try {
      const response = await axios.post(
        "http://localhost:8088/maintenance",
        newRecord
      );
  ...
  const handleAddRecord = async () => {
    try {
      ...
      const selectedGarage = garages.find(
        (garage) => garage.id === parseInt(newMaintenance.garageId, 10)
      );
      if (!selectedGarage) {
      ...
      const currentMaintenanceCount = maintenances.filter(
        (maintenance) =>
          maintenance.garageId === parseInt(newMaintenance.garageId, 10)
      ).length;

      // Проверете дали капацитетът ще бъде надвишен
      if (currentMaintenanceCount >= selectedGarage.capacity) {
        alert(
          `Cannot add maintenance. Garage capacity (${selectedGarage.capacity}) reached.`
        );
        return;
      }
  ...
  const handleUpdateRecord = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:8088/maintenance/${recordToUpdate.id}`,
        recordToUpdate
      );
  ...
  const handleDeleteMaintenance = async (maintenanceId) => {
    if (
      ...
      try {
        await axios.delete(
          `http://localhost:8088/maintenance/${maintenanceId}`
        );
  ...
  const filteredMaintenances = maintenances.filter((maintenance) => {
    ...
    if (startDate && new Date(maintenance.scheduledDate) < new Date(startDate))
      return false;

    if (endDate && new Date(maintenance.scheduledDate) > new Date(endDate))
      return false;
  ...
  const fetchMonthlyReport = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:8088/maintenance/monthlyRequestsReport",
        {
        ...
        for (let resp of response.data) {
          if (typeof resp.yearMonth.year != "undefined") {
            let yearMonth =
              resp.yearMonth.year +
              "-" +
              String(resp.yearMonth.monthValue).padStart(2, "0");
    ...
    <Container className="mt-4">
      <Card className="shadow-lg">
        ...
        <Card.Body>
          ...
          <h4 className="mt-4">
            {isUpdating ? "Update Maintenance Record" : "Add a New Record"}
          </h4>
          <Form>
            <Row>
              <Col md={4}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    ...
                    value={
                      isUpdating
                        ? recordToUpdate?.serviceType || ""
                        : newMaintenance.serviceType || ""
                    }
                    onChange={(e) =>
                      isUpdating
                        ? setRecordToUpdate((prev) => ({
                            ...prev,
            ...
            <Row className="align-items-end">
              ...
              <Col md={4}>
                <Form.Group className="mb-3">
                  ...
                  <ReactDatePicker
                    selected={
                      monthlyReportFilter.startMonth &&
                      /^\d{4}-\d{2}$/.test(monthlyReportFilter.startMonth)
              ...
              <Col md={4}>
                <Form.Group className="mb-3">
                  ...
                  <ReactDatePicker
                    selected={
                      monthlyReportFilter.endMonth &&
                      /^\d{4}-\d{2}$/.test(monthlyReportFilter.endMonth)
